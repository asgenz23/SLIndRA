...
## Combined Magnitude Map Product — Consolidated TR04 Update

For National/EEZ sea-level trend packages, include a combined sea-level change map when an approved boundary, gridded altimetry, and at least one QC-qualified UHSLC station are available.

### Product

- `product_id`: `TR04_magnitude_map_altimetry_tide_gauge`
- `product_family`: `Sea Level Trend`
- `product_role`: `required primary` when data and boundary requirements are met; otherwise `deferred`
- `plotting_helper`: `plot_magnitude_map`

### Inputs

Use `plot_magnitude_map` with:

- gridded CMEMS/Copernicus absolute altimetry trend magnitude in meters;
- approved EEZ/national boundary coordinates;
- tide-gauge station metadata and relative trend magnitude in meters;
- documented `vmin`, `vmax`, `xlims`, and `ylims`.

### Required records

Save the map image, map metrics table, boundary source/provenance record, product inventory row, validation checklist entry, and issue-log update. Captions must state that the background is absolute altimetry sea-level change and station markers are relative tide-gauge sea-level change. The quantities are displayed together but are not merged.

If a repository helper contains a hard-coded colorbar epoch, update only the displayed label after the helper call to reflect the actual analysis period. Do not alter the scientific calculation outside the approved workflow.
